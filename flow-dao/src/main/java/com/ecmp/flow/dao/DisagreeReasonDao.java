...
public interface DisagreeReasonDao extends BaseEntityDao<DisagreeReason> {
    ...
    @Query("select k from com.ecmp.flow.entity.DisagreeReason k where ( k.flowTypeId = :flowTypeId or k.flowTypeId = 'commonReason'  )  and  k.tenantCode = :tenantCode and k.status = true  order by k.rank  ")
