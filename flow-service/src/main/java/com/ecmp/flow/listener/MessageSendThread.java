...
//                    }
        ...
        if (notify != null) {
            ...
            JSONObject beforeNotify = notify.getJSONObject(eventType);
            String[] notifyType = {"notifyExecutor", "notifyStarter", "notifyPosition"};
            if (beforeNotify != null) {
//发送给流程的实际执行人
                JSONObject notifyExecutor = beforeNotify.getJSONObject("notifyExecutor");
                ...
                if (selectType != null && !selectType.isEmpty() && selectType.size() > 0) {
                    ...
                    for (Object type : types) {
                        if ("EMAIL".equalsIgnoreCase(type.toString())) {
                            ...
                            String currentTaskId = execution.getId();
                            FlowTask flowTask = flowTaskDao.findByActTaskId(currentTaskId);
                            if (flowTask == null || org.apache.commons.lang3.StringUtils.isEmpty(flowTask.getPreId())) {
                                preOpinion = "流程启动";
                            } else {
                                FlowTask flowTaskPre = flowTaskDao.findOne(flowTask.getPreId());
                                preOpinion = flowTaskPre.getDepict();
                            }
...
//发送给流程启动人
                JSONObject notifyStarter = beforeNotify.getJSONObject("notifyStarter");
...
//发送给选定的岗位
                JSONObject notifyPosition = beforeNotify.getJSONObject("notifyPosition");
